...
def write_eqs_E():
    eqs_E = '''
    dv / dt = (- g_m_E * (v - V_L) - I_syn - I_DC - I_sine) / C_m_E : volt (unless refractory)
    ...
    # Define the DC current
    I_DC = DC_input(t) : amp
    I_sine = sino_input(t) : amp
    '''
    return eqs_E
...
def write_eqs_I():
    eqs_I = '''
        dv / dt = (- g_m_I * (v - V_L) - I_syn - I_sine) / C_m_I : volt (unless refractory)
