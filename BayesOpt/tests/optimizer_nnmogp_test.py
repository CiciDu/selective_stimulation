...
class manifold_bo_optimizer(optimizer):
    def __init__(self, xy_start, proj_dim, objective, loss, **kwargs):
        ...
        self.identity = True
        self.binary_tree_list = self.build_tree_indices(
            D=self.Xnorm.shape[1] + 1)
    ...
    def initialize_modelM(self):
        k1 = gpflow.kernels.RBF(input_dim=self.proj_dim,
                                ARD=True, active_dims=list(range(self.proj_dim)))
        ...
        values = np.random.normal(loc=0., scale=1., size=[
                                  len(self.binary_tree_list)])
        ...
        nn_mogp = NN_MOGPR(X=np.copy(self.Xnorm),
                           Y=np.copy(self.Ynorm), kern=kc, nn=nn)
        nn_mogp.likelihood.variance = 0.0001

        K, f = nn_mogp.compute_log_likelihood()
        sio.savemat(
            '/home/rm4216/Desktop/ImperialCollege/Python/Github_manifold_bo/MATLAB/gabp-src/K.mat', {'K': K})
        sio.savemat(
            '/home/rm4216/Desktop/ImperialCollege/Python/Github_manifold_bo/MATLAB/gabp-src/f.mat', {'f': f})
        return kc, nn_mogp
    ...
    def run(self, maxiter=20):
        ...
        for j in range(maxiter):
            ...
            acquisition = initialize_acquisition_mo(
                loss=self.loss, gpmodel=nn_mogp, input_dim=self.input_dim, **kwargs)
...
defaults = ['0', 'Hartmann6D', 'manifold_bo', 'Neg_ei', int(2), int(
    6), int(50)]    # list ints, name_objective, name_optimizer, name loss
