...
#
# Xnn = np.load('/Users/dusiyi/Downloads/Pulkit_class/final_project/selective_stimulation/BayesOpt/tests/multioutput_test/'
#               'Xnn_sample.npy')
# X = np.load('/Users/dusiyi/Downloads/Pulkit_class/final_project/selective_stimulation/BayesOpt/tests/multioutput_test/X_'
#             'sample.npy')
# Y = np.load('/Users/dusiyi/Downloads/Pulkit_class/final_project/selective_stimulation/BayesOpt/tests/multioutput_test/Y_'
#             'sample.npy')
...
with gpflow.defer_build():
    ...
    feature_list = [gpflow.features.InducingPoints(
        Xnn[:M, :]) for r in range(rank)]
...
f, axs = plt.subplots(
    X.shape[1] + 1, 1, figsize=(8, 4*X.shape[1]), sharex=True)
x_num = np.arange(Xnn.shape[0])
# XY = X
labels_y = ['x0', 'x1', 'x2', 'x3', 'x4', 'y']
for i, ax in zip(range(X.shape[1] + 1), axs):
    ax.errorbar(x_num, mean_X[:, i], yerr=2*np.sqrt(var_X[:, i]),
                fmt='o', color='blue', label='mogp x')
