...
for i in range(500):
    for j in range(n_intervals):
        assert Ynorm_gp[j, i] == (
            data_obs[j, i] - obs_mean[j, :])/obs_std[j, :]
...
class SwissRoll1D(object):
    ...
    def f(self, x, noisy=False, fulldim=False):
        ...
        f_x = np.multiply(
            self.Y[indices_min, :], np.exp(-pw_dists2[indices_m, indices_min])[:, None])

        if noisy:
            # noise = np.random.normal(loc=0., scale=self.scale, size=f_x.shape[0])[:, None]
            noise = np.array([random.gauss(0, self.scale)
                             for _ in range(f_x.shape[0])])[:, None]
            y = f_x + noise
            return y
        return f_x    # Nx1
...
#
#
# path_to_data = '/Users/dusiyi/Downloads/Pulkit_class/final_project/selective_stimulation/BayesOpt/datasets/data/' + dict_args['obj'] + '/'
#
...
# the point of this form is to store the order, lists preserve the order
names = ['seed', 'obj', 'n_samples']
defaults = ['0_1_2_3_4_5_6_7_8_9_10_11_12_13_14_15_16_17_18_19',
            'SwissRoll1D', int(50)]
types = [str, str, int]
parser = argparse.ArgumentParser()
for name_i, default_i, type_i in zip(names, defaults, types):
...
verify_attr(dict_args)    # throw error if there are None values
path_to_transform_inputs = 'tfbo/utils/check_inputs'
transform_attr = import_attr(
    path_to_transform_inputs, attribute='transform_inputs')
dict_args['seed'] = transform_attr(dict_args['seed'])


parts = [int(10), int(11), int(12), int(13), int(
    14), int(15), int(16), int(17), int(18), int(19)]

# passing an iterable preserves the order
dict_i = OrderedDict([(name_i, dict_args[name_i]) for name_i in names])
...
for part_i in parts:
    for seed_i in dict_args['seed']:
        ...
        indices_selection_i = np.random.permutation(
            np.arange((sr1D.Npoints/20)*(part_i+1)))[:dict_args['n_samples']]

        _x = sr1D.X[indices_selection_i.astype(int), :][None]
        _x01 = (_x - sr1D.x_bounds[0]) / (sr1D.x_bounds[1] - sr1D.x_bounds[0])
        _fx = sr1D.f(_x01[0, :, :], noisy=False, fulldim=False)
        assert np.max(
            np.abs(_fx - sr1D.Y[indices_selection_i.astype(int), :])) <= 1e-012
        _noise = np.random.normal(
            loc=0., scale=sr1D.scale, size=_fx.shape[0])[:, None]
        _y = _fx + _noise   # same shape

        x_file, y_file = name_attr(dict_i)

        path_to_data = '/Users/dusiyi/Downloads/Pulkit_class/final_project/selective_stimulation/BayesOpt/datasets/data/' + dict_args[
            'obj'] + '/' + 'part' + str(part_i) + '/'
        # str.replace is not in-place modificattion of filename
        save_attr(path_to_data + x_file + '_' + 'part' +
                  str(part_i) + '.npy', array=_x01)
        save_attr(path_to_data + y_file + '_' + 'part' +
                  str(part_i) + '.npy', array=_y)
