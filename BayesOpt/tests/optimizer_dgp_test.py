...
sys.path.insert(
    0, "/Users/dusiyi/Downloads/Pulkit_class/final_project/selective_stimulation/BayesOpt/Doubly_Stochastic_DGP")
sys.path.insert(
    0, "/homes/rm4216/Desktop/ImperialCollege/Python/Github_manifold_bo/BayesOpt/Doubly_Stochastic_DGP")
...
class manifold_bo_optimizer(optimizer):
    ...
    def initialize_modelM(self):
        ...
        kernels = []
        for dim_i in dims:
            kernels.append(gpflow.kernels.RBF(input_dim=dim_i, ARD=True, lengthscales=np.ones(
                shape=[dim_i]) * 0.2))    # not relevant any more
    ...
    def train_model(self, dgp_model):
        ...
        ng_action = NatGradOptimizer(gamma=0.1).make_optimize_action(
            dgp_model, var_list=ng_vars)
    ...
    def update_latent_bounds(self, dgp_model, opt_config):
        ...
        self.latent_grid = np.multiply(
            np.copy(self.grid), m_max - m_min) + m_min
        return opt_config

    def generate_x(self, x_proj, dgp):
        sample, x, var = dgp.predict_last_layer(x_proj, 1)
        x_new = (x[0, :, :self.input_dim].copy() * self.X_std) + \
            self.X_mean     # originally mapped to Xnorm
    ...
    def run(self, maxiter=20):
        ...
        for j in range(maxiter):
            ...
            acquisition = initialize_acquisition_dgp(
                loss=self.loss, gpmodel=dgp_model, num_samples=int(1), **kwargs)
    ...
    def minimize_acquisition(self, acquisition, opt_config):
        ...
        x_topk = np.ravel(
            np.copy(self.latent_grid[indices_sorted[:self.num_init, 0], :]))
    ...
    def get_hyps(self, dgp):
        ...
        # gp.kern.kernels[0].lengthscales.read_value()
        lengthscales_list = list(map(lengthscales, dgp.layers))
        ...
        # gp.kern.kernels[0].variance.read_value()[None]
        kv = list(map(amplitude, dgp.layers))
...
defaults = ['0', 'Hartmann6D', 'dgpMo_bo', 'Neg_ei', int(2), int(6), int(10)]
