...
def loadMat(filename):
    path = '/Users/dusiyi/Downloads/Pulkit_class/final_project/selective_stimulation/BayesOpt/datasets/tasks/'
    try:
...
def load_partname(path, partname):
    path_to_os = '/Users/dusiyi/Downloads/Pulkit_class/final_project/selective_stimulation/BayesOpt'
    try:
        os.chdir(path_to_os)
...
def load_initializations(dict_args, names):
    ...
    attr_partload = import_attr(
        'tfbo/utils/load_save', attribute='load_partname')
...
def load_SRinitializations(dict_args, names):
    ...
    name_attr = import_attr('tfbo/utils/name_file',
                            attribute='name_file_start')
    path = 'datasets/data/' + dict_args['obj'] + \
        '/' + 'part' + str(dict_args['part']) + '/'
    attr_load = import_attr('tfbo/utils/load_save', attribute='loadfile')
    full_path = '/Users/dusiyi/Downloads/Pulkit_class/final_project/selective_stimulation/BayesOpt/' + path

    def load_pair(seed_i):
        dict_start['seed'] = seed_i
        x_file, y_file = name_attr(dict_start)
        y_start = attr_load(filename=full_path + y_file +
                            '_' + 'part' + str(dict_args['part']) + '.npy')
        x_start = attr_load(filename=full_path + x_file +
                            '_' + 'part' + str(dict_args['part']) + '.npy')
        return (x_start, y_start)
    xy_list = list(map(load_pair, dict_args['seed']))
    return xy_list
