...
names = ['seed', 'obj', 'opt', 'loss',
         'proj_dim', 'input_dim', 'maxiter', 'part']
defaults = ['0', 'SwissRoll1D', 'SRNNKL_bo',
            'Neg_ei', int(1), int(2), int(5), int(10)]
types = [str, str, str, str, int, int, int, int]
helps = ['seed_number', 'objective_name', 'optimizer_name',
         'loss_name', 'proj_dim', 'input_dim', 'maxiter', 'proportion_of_SR']
...
for name_i, default_i, type_i, help_i in zip(names, defaults, types, helps):
    parser.add_argument('--' + name_i, default=default_i,
                        type=type_i, help=help_i)
...
# load start
load_start_attr = import_attr(
    'tfbo/utils/load_save', attribute='load_SRinitializations')
...
if dict_args['input_dim'] == dict_args['proj_dim']:
    xy_list_input_dim = [(xy_list[i][0][:, :, objective.relevant_dims],
                          xy_list[i][1]) for i in range(len(xy_list))]
    y_list = []
...
def optimizer_run(xy_i, seed_i):
    ...
    optimizer = optim_attr(
        xy_i, proj_dim=dict_args['proj_dim'], objective=objective, loss=dict_args['loss'], **kwargs)
...
save_dictionary(path_dict + filename + '_part' +
                str(dict_args['part']) + 'seed_' + str(dict_args['seed'][0]) + '.p', dict_out)
