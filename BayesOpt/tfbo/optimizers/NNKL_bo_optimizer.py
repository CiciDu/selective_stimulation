...
class NNKL_bo_optimizer(optimizer):
    ...
    def generate_x(self, x_proj, gp_joint):
        ...
        m = 0.
        v = 1.
        # Equation (3.82) GPML book = Equation (3.25) GPML book
        z = (joint_new - m) / (v * np.sqrt(1. + joint_var / (v ** 2.)))
        x_out = norm.cdf(z)
    ...
    def NLconstraint(self, opt_config):
        ...
        def KLconstraint(self, x):
            ...
            max_mean_components = np.amax(np.abs(x_mean_fast), axis=1)[
                :, None]  # M x 1
            rterm = max_mean_components ** 2. / \
                (2 * self.L**2)                  # M x 1
            # M x 1
            lterm = np.min(KL, axis=1)[:, None]
            constraint = lterm - rterm
            return constraint[:, 0]

        def f_constraint(x): return KLconstraint(self=self, x=x)
        opt_config['constraints'] = NonlinearConstraint(
            f_constraint, lb=-np.inf, ub=0, jac='2-point')
        return opt_config

    def run(self, maxiters=20):
        # check import configuration
        opt_config = import_attr(
            'tfbo/configurations', attribute='KLacquisition_opt')
        for j in range(maxiters):
            ...
            acquisition = block_diag_initialize_acquisition(
                loss=self.loss, gpmodel=gp_nnjoint, **kwargs)
            # opt_config = self.update_latent_bounds(opt_config)
            if (j % 10 == 0):
                # learn new Lipschitz constant every 10 iters
                self.learnLipschitz(gp_nnjoint)
