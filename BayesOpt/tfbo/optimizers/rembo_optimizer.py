...
class rembo_optimizer(optimizer):
    ...
    def initialize_A(self, identity=False):
        ...
        if self.input_dim == 12:
            path_load = '/Users/dusiyi/Downloads/Pulkit_class/final_project/selective_stimulation/BayesOpt/Baselines/rembo/demos/ElectronSphere6np/A_orth_matrices/'
            from scipy.io import loadmat
            try:
                path_orth_A = '/Users/dusiyi/Downloads/Pulkit_class/final_project/selective_stimulation/BayesOpt/Baselines/rembo/demos/ElectronShpere6np/A_orth_matrices/A_orth_seed=' + \
                    str(self.seed + 1) + '.mat'
            ...
            # q has shape (self.input_dim, self.proj_dim)
    ...
    def in_bounds(self, x_proj_minimizer):
        ...
        def in_bounds(index): return in_bound_i(
            components=x_proj_minimizer, index=index, self=self)
        # a bool for each dimension in the original space
        list_bools = list(map(in_bounds, list(range(self.proj_dim))))
        return all(list_bools)

    def linear_map(self, x_proj):                   # Ax_proj=x  <=>  embedd -> original_space
        # x_proj.shape:  N x d (embedded space)
        ...
        # ambient space [-1, 1]
        xm11 = np.matmul(x_proj, np.transpose(self.A))
        ...
        # ambient space in [-1, 1] -> ambient space in [0, 1] and clip
        return np.clip((xm11 + 1.) / 2., a_min=0., a_max=1.)

    # x_proj=pinv(A)x  <=>  original_space -> embedd
    def inverse_linear_map(self, x):
    ...
    def update_data(self, x_proj, y):
        ...
        self.data_x = np.concatenate(
            [self.data_x, x], axis=0)     # check shapes
    ...
    def run(self, maxiters):
        ...
        for i in range(maxiters):
            ...
            acquisition = initialize_acquisition(
                self.loss, gpmodel=gp, **kwargs)   # updated model at each iteration
            ...
            def acquisition_normalized(x): return self.acquisition_norm(
            ...
            try:
                x_tp1, acq_tp1 = self.minimize_acquisition(
                    acquisition_normalized, opt_config)    # check configuration, starting point
            except:
    ...
    def initialize_normalization(self):
        ...
        self.X_proj_norm = (self.data_x_proj - self.X_proj_mean) / \
            self.X_proj_std  # check shapes

        def test_normalization(input_norm):
            xm_test = np.mean(input_norm, axis=0, keepdims=True)
            xst_test = np.std(input_norm, axis=0, keepdims=True) - \
                np.ones([1, input_norm.shape[1]])
            boolm_list = [
                np.abs(xm_i[0]) <= 1e-02 for xm_i in list(xm_test.transpose())]
            bools_list = [
                np.abs(xs_i[0]) <= 1e-02 for xs_i in list(xst_test.transpose())]
