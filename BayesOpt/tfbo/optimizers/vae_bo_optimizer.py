...
class vae_bo_optimizer(optimizer):
    ...
    def decode_zopt(self, z_opt):
        ...
        # first bring to interval [-1, 1] then evaluate the cdf function
        x_out = norm.cdf(x_opt.astype(np.float64))
        return x_out

    def run(self, maxiters):
        # load VAE models for a specific objective function
        path = '/Users/dusiyi/Downloads/Pulkit_class/final_project/selective_stimulation/BayesOpt/Baselines/chemvae/' + \
               self.dict_args['obj'] + '/'
        ...
        for i in range(maxiters):
            ...
            try:
                z_tp1, acq_tp1 = self.minimize_acquisition(
                    acquisition, opt_config)    # check configuration, starting point
            except:
                gp = self.reset_hyps(gp)
                z_tp1, acq_tp1 = self.minimize_acquisition(
